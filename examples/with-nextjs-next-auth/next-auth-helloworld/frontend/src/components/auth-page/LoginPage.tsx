...
export const LoginPage: React.FC<LoginProps> = ({
  ...
  const CardTitle = <Typography.Title level={3}>helloworld</Typography.Title>;
    ...
    if (providers && providers.length > 0) {
      return (
        <>
          ...
          {!hideForm && (
            <Divider>
              <Typography.Text>{translate('pages.login.divider', 'or')}</Typography.Text>
            </Divider>
          )}
        </>
      );
  ...
  const CardContent = (
    <Card title={CardTitle} {...(contentProps ?? {})}>
      {/* {renderProviders()} */}
      <Form
        layout="vertical"
        form={form}
      ...
      >
        ...
        >
          <Input type="password" autoComplete="current-password" placeholder="●●●●●●●●" size="large" />
        </Form.Item>
        <div
          style={{
            display: 'flex',
